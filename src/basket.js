class Basket {
  ...
  addItem(name, price) {
    ...
    console.log(
      `${name} has been successfully added. Your basket is starting to look like a treasure trove!`
    );
  }
  ...
  removeItem(itemToRemove) {
    ...
    if (index !== -1) {
      ...
      console.log(
        `Unable to find ${itemToRemove}... Are you sure it was in the basket, or is it just an illusion?`
      );
    }
  }
  ...
  emptyBasket() {
    this.items = [];
    console.log(
      "The basket is now as empty as a salesman's promises on Black Friday."
    );
  }
}
